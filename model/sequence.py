...
class PoseSequence:
    def __init__(self, npz_file):
        with np.load(npz_file) as data:
            ...
            # self.trans = 2 * (self.trans - np.min(self.trans)) / (np.max(self.trans) - np.min(self.trans)) - 1
            if "fps" in data.files:
                self.fps = data["fps"]
            else:
                self.fps = data["mocap_framerate"]
                # self.source = data["source"]
            if len(np.shape(self.poses)) == 2:
                self.poses = np.reshape(self.poses, (np.shape(self.poses)[0], int(np.shape(self.poses)[1] / 3), 3))
            if self.poses.shape[-1] == 3:
                self.poses = axis_angle_to_quat(self.poses)
            if np.shape(self.poses)[1] == 52:  # smplh
                self.poses = self.poses[:, :24, :]
            self.poses[:, 22:24, :] = 0
