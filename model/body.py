...
class Body:
    def __init__(self, body_model, input_joints=None):
        ...
        if n_dims == 3:
            ...
            raise ValueError(f"Unsupported rest pose shape: {self.rest_pose.shape}")
    ...
    def forward_kinematics(self, rotations_basis, root_locations):
        rotations_basis = tf.cast(rotations_basis, tf.float32)
        root_locations = tf.cast(root_locations, tf.float32)
        input_shape = tf.shape(rotations_basis)
        ...
        for depth in self.depth:
            ...
            rotation_parents = tf.stack([rotations[p] for p in parents], axis=1)

            rot = tf.einsum("abcd,bde->abce", rotation_parents, self.rotations_local_inv[parents])
            rot = tf.einsum("abcd,bde->abce", rot, self.rotations_local[depth])
            rot = tf.einsum("abcd,abde->abce", rot, tf.gather(rotations_basis, depth, axis=1))

            for i, n in enumerate(depth):
                rotations[n] = rot[:, i]

        rotations = tf.stack(rotations, axis=1)
        rotations = tf.einsum("abcd,bde->abce", rotations, self.rotations_local_inv)[:, :, :3]

        final_shape = tf.concat([batch_shape, [self.num_joints, 3, 4]], axis=0)
        return tf.reshape(rotations, final_shape)
