...
from utils.mesh import (
    ...
    dihedral_dir_angle_adjacent_faces,
    edge_lengths,
    edge_dir_adjacent,
    face_normals,
    laplacian_matrix,
...
class Garment:
    def __init__(self, obj):
        ...
        (
            ...
            self.edge_adjacency_index,
            self.max_vertices_neighbours_count,
            self.vertices_neighbours,
            self.vertices_neighbours_edges,
            self.max_edges_neighbours_count,
            self.edges_neighbours
        ) = faces_to_edges_and_adjacency(self.faces)
        self.edge_adjacency_dir = edge_dir_adjacent(self.edges, self.edge_adjacency_index, self.vertices)
        ...
        print("\tComputing direct adjacent face dihedral angles...")
        self.face_dir_dihedral = dihedral_dir_angle_adjacent_faces(
            self.normals, self.face_adjacency, self.edge_adjacency_dir
        )
    ...
    def make_continuum(self):
        ...
        axis /= np.linalg.norm(axis, axis=-1, keepdims=True) + np.finfo(np.float32).eps
